...
def extract_contacts(text):
    ...
    address_keywords = ['street', 'st.', 'road', 'rd.', 'avenue', 'ave', 'blvd', 'drive', 'dr.', 'lane', 'ln.', 'court', 'ct.', 'circle', 'square', 'building', 'unit', 'floor']
...
if __name__ == "__main__":
    website = "https://app.apollo.io/#/organizations/556988247369642529854f00"            #url
    scrape_depth = 0
    ...
    if enable_contact_extraction:
        contact_file = os.path.join(output_dir, "contacts.txt")
        with open(contact_file, "w", encoding="utf-8") as contact_out:
            for url, text in scraped_data:
                phones, emails, addresses = extract_contacts(text)
                contact_out.write(f"URL: {url}\n")
                if phones:
                    contact_out.write("Phones:\n" + "\n".join(set(phones)) + "\n")
                if emails:
                    contact_out.write("Emails:\n" + "\n".join(set(emails)) + "\n")
                if addresses:
                    contact_out.write("Addresses:\n" + "\n".join(set(addresses)) + "\n")
                contact_out.write("=" * 100 + "\n\n")
        print("Contacts saved to output/contacts.txt")
