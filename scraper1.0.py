...
def log_error(context: str, url: str = "", exc: Exception = None):
...
parser = argparse.ArgumentParser(description="SCRAPER1.0 Web Crawler")
parser.add_argument("-hl", "--headless", action="store_true", default=True)
parser.add_argument("-u", "--url", type=str)
parser.set_defaults(contacts=True, txt=True)
contacts_group = parser.add_mutually_exclusive_group()
contacts_group.add_argument("-c", "--contacts", dest="contacts", action="store_true")
contacts_group.add_argument("-nc", "--no-contacts", dest="contacts", action="store_false")
txt_group = parser.add_mutually_exclusive_group()
txt_group.add_argument("-txt", "--txt", dest="txt", action="store_true")
txt_group.add_argument("-nt", "--no-txt", dest="txt", action="store_false")
parser.add_argument("-d", "--depth", type=int, default=2)
parser.add_argument("-pdf", "--pdf", action="store_true", default=False)
parser.add_argument("-da", "--download-all", action="store_true", default=False)
parser.add_argument("-ss", "--screenshot", action="store_true", default=False)
parser.add_argument("-mo", "--mayor-only", action="store_true")
parser.add_argument("-cb", "--combine", action="store_true", default=False)
...
main_domain_group.add_argument("-md", "--main-domain", dest="main_domain", action="store_true")
main_domain_group.add_argument("-nmd", "--no-main-domain", dest="main_domain", action="store_false")
...
def download_file(url, save_dir):
    try:
        ...
        if r.status_code == 200:
            ...
            console.print(f"Failed to download (HTTP {r.status_code}): {url}", style="bold red")
    except Exception as e:
        console.print(f"Failed to download: {url}", style="bold red")
        log_error("download_file", url, e)
...
def download_pdf(url, save_dir):
    try:
        ...
        if r.status_code == 200:
            ...
            console.print(f"Failed to download PDF (HTTP {r.status_code}): {url}", style="bold red")
    except Exception as e:
        console.print(f"Failed to download PDF: {url}", style="bold red")
        log_error("download_pdf", url, e)
...
def looks_like_name(local):
    ...
    if any(kw == clean_local.lower() or clean_local.lower().startswith(kw) for kw in non_name_keywords):
        return False
...
def force_fetch_after_block(playwright, url, screenshot_dir=None):
    browser = None
    try:
        browser = playwright.chromium.launch(headless=FORCE_FETCH_HEADLESS or use_headless_browser, args=["--disable-blink-features=AutomationControlled"])
        ...
        console.print(f"[FORCE-FETCH] Failed", style="bold red")
        log_error("force_fetch_after_block", url, e)
        return "", set()
    finally:
        if browser:
            browser.close()
...
def solve_bot_and_get(playwright, url, screenshot_dir=None):
    ...
    try:
        ...
        console.print(f"[BOT-DETECTION] Failed to solve bot page", style="bold red")
        log_error("solve_bot_and_get", url, e)
        return "", set()
    finally:
        if browser:
            browser.close()
...
def get_page_text_and_links(playwright, url, screenshot_dir=None):
    ...
    try:
        ...
        console.print(f"Failed to scrape {url}", style="bold red")
        log_error("get_page_text_and_links", url, e)
        try:
            return force_fetch_after_block(playwright, url, screenshot_dir)
        except Exception as e2:
            log_error("get_page_text_and_links:force_fetch_fallback", url, e2)
            return "", set()
    finally:
        if browser:
            browser.close()
...
def basic_request_text_and_links(playwright, url, screenshot_dir=None):
    if enable_image_download:
        return get_page_text_and_links(playwright, url, screenshot_dir)
    if any(keyword in url.lower() for keyword in SKIP_KEYWORDS):
        console.print(f"[SKIP BROWSER] URL contains skip keyword: {url}", style="bold yellow")
        return "", set()
    browser = None
    try:
        try:
            ...
            console.print(f"[HEAD ERROR] Try force fetch: {url}", style="bold yellow")
            log_error("basic_request_text_and_links:head", url, e)
            return force_fetch_after_block(playwright, url, screenshot_dir)
        browser = playwright.chromium.launch(headless=use_headless_browser, args=["--disable-blink-features=AutomationControlled"])
        context = browser.new_context(user_agent=UA, viewport={"width": 1, "height": 1})
        page = context.new_page()
        ...
        try:
            ...
            log_error("basic_request_text_and_links:wait_ready", url, e)
        ...
        console.print(f"Failed to scrape {url}", style="bold red")
        log_error("basic_request_text_and_links", url, e)
        try:
            return force_fetch_after_block(playwright, url, screenshot_dir)
        except Exception as e2:
            log_error("basic_request_text_and_links:force_fetch_fallback", url, e2)
            return "", set()
    finally:
        if browser:
            browser.close()
    ...
    for img in img_tags:
        ...
        try:
            ...
            if response.status_code == 200 and response.content:
                ...
                console.print(f"[IMG] Failed: {img_url}", style="bold red")
        except Exception as e:
            console.print(f"[IMG] Failed: {img_url}", style="bold red")
            log_error("save_all_images_from_page", img_url, e)
...
def scrape_with_depth(playwright, start_url, max_depth, contact_file=None):
    ...
    while to_visit:
        ...
        for current_url, depth in current_level_tasks:
            ...
            if mayor_only_filter or enable_image_download:
...
def print_config():
    ...
    console = Console()
    console.print(fig.renderText('SCRAPER1.0'), style="bold red")
    config_items = [
        ("Depth", scrape_depth),
...
def scrape_one_site_single(website):
    ...
    try:
        ...
        try:
            ...
            log_error("scrape_one_site_single:playwright.stop", website, e)
...
def scrape_one_site_inner(website, playwright):
    try:
        ...
        text_data, seen_emails, seen_phones = scrape_with_depth(playwright, website, scrape_depth, contact_file)
        ...
        log_error("scrape_one_site_inner", website, e)
    ...
    try:
        ...
        with fail_lock:
            ...
            with open(fail_log_path, "a", encoding="utf-8") as f:
                f.write(website + "\n")
        print(f"[ERROR] {website}")
        log_error("scrape_one_site", website, e)
    finally:
        try:
            playwright.stop()
        except Exception as e:
            log_error("scrape_one_site:playwright.stop", website, e)

def print_config_and_prepare():
    ensure_output_dir(output_dir)
    print_config()

if __name__ == "__main__":
    print_config_and_prepare()
    urls = []
    try:
        if args.url:
                ...
                with ThreadPoolExecutor(max_workers=max_threads) as executor:
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            print(f"[ERROR] Thread failed")
                            log_error("main:thread", "", e)
